...
export const YoungAdultModule: React.FC<YoungAdultModuleProps> = ({ onComplete }) => {
    ...
    const corpus25 = calculateCorpus(25, 1000); // Baseline comparison
        ...
        <div className="space-y-6">
            <div className="bg-indigo-50 border-l-4 border-indigo-500 p-4 rounded">
                <h3 className="font-bold text-indigo-800 flex items-center gap-2">
                    <TrendingUp size={20} /> Mission: Time Machine
                </h3>
                <p className="text-sm text-indigo-700">See how starting early makes you a Crore-pati. Visualize your retirement.</p>
            </div>

            <div className="space-y-6">
                <div>
                    <label className="block text-sm font-bold text-slate-700 mb-2">Start Age: {age} years old</label>
                    <input
                        type="range"
                        min="20"
                        max="50"
                ...
                <div>
                    <label className="block text-sm font-bold text-slate-700 mb-2">Monthly Investment (SIP): ₹{monthlyInvest}</label>
                ...
                <div className="bg-slate-900 text-white p-6 rounded-xl shadow-inner text-center">
                    <p className="text-slate-400 text-sm uppercase tracking-wide mb-2">Corpus at Age 60</p>
                    <div className="text-3xl md:text-4xl font-bold text-green-400">
                        ₹{(currentCorpus / 100000).toFixed(2)} Lakhs
                    </div>
                    <div className="text-xs text-slate-500 mt-2">Assuming 12% annual returns</div>
                </div>
                ...
                <div className="p-4 bg-yellow-50 rounded-lg text-sm text-yellow-800 flex items-start gap-2">
                    ...
                    <p><strong>Lesson:</strong> The earlier you start, the more "Magic of Compounding" works for you. Starting at 20 vs 30 makes a massive difference.</p>
                </div>
            </div>
        </div>
    );
